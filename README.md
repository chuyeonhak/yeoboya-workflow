...
{
  "plugins": {
    "paths": ["/path/to/yeoboya-work-flow"]
  }
}
```
...
```bash
cd /path/to/yeoboya-work-flow/skills/pdf-spec-organizer/scripts
python3 -m venv .venv
...
4. 플러그인이 **피처 DB 자동 생성** + `yeoboya-work-flow.config.json` 초안 출력
...
- `yeoboya-work-flow.config.json` 의 `notion_data_source_id` 로 피처 DB 조회
- 동명 피처 존재 시:
...
### `yeoboya-work-flow.config.json` (프로젝트 레포에 커밋)
...
{
  "pdf_spec_organizer": {
    "notion_database_id": "45e4c4b2-990f-4327-bc64-01b348f34df0",
    "notion_data_source_id": "00702d09-f933-41a1-8d89-698de45c963c",
    "parent_page_id": "31a20097-6459-80e1-bd2c-d9628e849ab9"
  }
}
```
...
| 증상 | 원인 | 해결 |
|---|---|---|
| `❌ 팀 공유 설정 파일이 없습니다` | `yeoboya-work-flow.config.json` 없음 | 팀 리드가 최초 셋업 실행 후 JSON 을 레포에 커밋 |
...
`yeoboya-work-flow` 는 umbrella 플러그인이라 기능을 계속 쌓을 수 있게 설계됨:
