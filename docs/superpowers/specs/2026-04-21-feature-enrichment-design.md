...
- 코드베이스 탐색은 **Claude Code `Explore` subagent** 로 수행 (자연어 프롬프트 기반, grep/fixed keywords 대체)
- 여러 피처 탐색은 `superpowers:dispatching-parallel-agents` 원칙으로 병렬화
...
`codebase_roots` 설정된 플랫폼에 대해 **Claude Code `Explore` subagent** 를 spawn 해서 자연어로 탐색:

**Explore subagent 호출 구조:**
```
Agent(
  subagent_type="Explore",
  description="<피처명> 관련 코드 조사",
  prompt="""
  프로젝트 루트: <codebase_roots.ios 또는 codebase_roots.android>
  thoroughness: medium

  다음 피처가 이미 존재하거나 관련 구조가 있는지 조사하고,
  새로 구현할 경우의 복잡도를 요약해서 보고해달라.
  ...
  2. 재사용 가능 컴포넌트 / 보일러플레이트 있는지
  3. 추가 작업 대비 신규 파일/모듈 수 대략
  4. 기존 코드 패턴과 충돌 가능성
  5. 요약은 200단어 이내
  """
)
...
- **병렬화 규칙 (`superpowers:dispatching-parallel-agents` 원칙):**
  - 피처 3개 미만 → 순차 호출 (부하 낮음)
  - 피처 3개 이상 → 플랫폼별로 묶어 병렬 dispatch (iOS 피처끼리, Android 피처끼리)
  - 공통 피처는 iOS/Android 양쪽 각각 Explore 호출 (결과 합쳐서 Claude 메타 생성)
- **`thoroughness="medium"`** 기본값. 초과 토큰 리스크 감지 시 `"quick"` 으로 자동 강등 (오픈 이슈)
- **실패 처리:** Explore 가 에러 반환 또는 timeout (2분) → 해당 피처는 탐색 결과 없이 메타 생성 + 경고
- **경로 검증:** `codebase_roots` 에 지정된 경로가 존재하지 않으면 최초 1회만 경고 후 해당 플랫폼 탐색 스킵

Explore 결과(200 단어 요약)는 "관련 코드 탐색 결과" 로 Claude 메타 생성 프롬프트에 포함.
...
[관련 코드 탐색 결과]  # codebase_roots 설정 시만, Explore subagent 보고 요약
<Explore subagent 보고 (플랫폼별, 200단어 이내)>
...
| Explore subagent timeout (2분) 또는 오류 | 해당 피처 탐색 결과 없이 메타 생성 + 경고 |
| Explore 토큰 한도 초과 감지 | 이후 피처부터 thoroughness="quick" 자동 강등 |
...
| `skills/pdf-spec-organizer/scripts/enrich_features.py` | 신규 | project-context 로드, features.json 의 `metadata` 필드 병합, JSON 파싱/fallback. (Explore subagent 호출은 SKILL.md 의 Claude 지시로 수행 — Python 스크립트는 I/O 전담) |
...
| `skills/pdf-spec-organizer/scripts/tests/test_enrich_features.py` | 신규 | pytest: JSON 파싱 fallback, project-context 절삭, features.json metadata 병합. Explore subagent 호출은 SKILL.md 영역이라 단위 테스트 대상 아님 (수동 시나리오로 검증) |
...
- `enrich_features.py`:
  ...
  - features.json `metadata` 병합 로직
  - JSON 파싱 성공/실패 처리
  - 빈 메타 fallback 동작
- Explore subagent 호출 자체는 SKILL.md 에서 Claude 가 Agent 툴로 수행하므로 pytest 대상 아님. 수동 시나리오(아래) 로 검증.
...
- **토큰 비용:** 피처 N 개마다 Explore subagent 1-2회 + Claude 메타 생성 1회. 피처 10개 이상 PDF 에서 비용 증가. project-context 배치 처리, Explore 결과 캐싱, thoroughness 동적 조절 방식 검토 (v0.5+)
- **Explore thoroughness 자동 강등 트리거:** "토큰 한도 초과 감지" 의 구체적 기준 (예: 누적 토큰 80% 도달) 은 실측 후 v0.5 에서 임계값 고정
