...
|---|---|---|---|---|
| 1 | 사전 준비 확인 | Notion MCP 연결, Tesseract 설치, Superpowers 플러그인 로드됨 | `brew install tesseract tesseract-lang` (Tesseract 만 해당) | 3개 모두 OK |
...
README 상단 (line 5 ~ 7 사이, "상태:" 줄 바로 아래) 에 1 줄 추가:
...
- [ ] CDN 차단 환경 (캐시 클리어 + 오프라인) 에서도 사전에 캐시된 경우 로드됨
...
- 다국어 (영어)
- 실습/데모 섹션 — 샘플 PDF + 인터랙티브 Phase walkthrough
