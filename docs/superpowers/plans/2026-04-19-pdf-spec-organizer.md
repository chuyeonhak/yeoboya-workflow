...
**Goal:** `yeoboya-work-flow` 플러그인의 첫 기능 `pdf-spec-organizer` 를 구현한다. 복합 PDF 를 파싱해 Notion 피처 DB 페이지로 퍼블리시하고, 명세 누락 체크 + iOS/Android 플랫폼별 개발자 노트를 공유할 수 있게 한다.
...
> **참고:** `yeoboya-work-flow.config.json` (팀 공유 DB ID) 은 **플러그인 레포가 아닌** 플러그인을 사용하는 프로젝트 레포의 루트에 커밋됩니다. 이 플랜에서는 README 에 가이드만 문서화합니다.
...
```bash
cd /Users/chuchu/testPlugin && git init 2>/dev/null; git add .claude-plugin/plugin.json && git commit -m "chore: initialize yeoboya-work-flow plugin manifest"
```
...
- 기존 source-of-truth 구성:
  ...
  ```
  프로젝트 레포의 `yeoboya-work-flow.config.json` 에 선언.
...
(미리 `yeoboya-work-flow.config.json` 을 테스트용 Notion DB 가리키게 준비)
